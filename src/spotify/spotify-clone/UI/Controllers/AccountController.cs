...
public class AccountController : Controller
{
    ...
    public static readonly string SpotifyApi= "http://spotify-api:80/";
    ...
    public async Task<IActionResult> LogIn(string email, string password, bool remember)
    {
        ...
        var res = client
            .PostAsync($"{SpotifyApi}api/auth/login", content).Result;
        ...
        int year, int month, int day)
    {
        ...
        var content = new FormUrlEncodedContent(values);
        var res = client
            .PostAsync($"{SpotifyApi}api/auth/signup", content).Result;
